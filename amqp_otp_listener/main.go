...
var (
	...
	exchange     = flag.String("exchange", amqpHelper.SMSExchange, "The exchange we will be binding to")
	exchangeType = flag.String("exchange_type", "direct", "Type of exchange we are binding to | topic | direct| etc..")
	queue        = flag.String("queue", amqpHelper.SMSIncoming, "Name of the queue that you would like to connect to")
	routingKey   = flag.String("routing_key", amqpHelper.SMSDeadLetter, "queue to route messages to")
...
func handler(deliveries <-chan amqp.Delivery, uc *usecase.UcContract) {
	...
	for d := range deliveries {
		...
		otpUc := usecase.OtpUseCase{UcContract: uc}
		res,err := otpUc.SendOTP(formData["mobilePhone"].(string))
		if err != nil {
